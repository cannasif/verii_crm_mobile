...
export function SearchFieldSelectorButton({ options, selectedFields, onChange, compact = false }: Props): React.ReactElement {
  ...
  return <>
    <TouchableOpacity style={[styles.trigger, compact && styles.triggerCompact, { borderColor: colors.border, backgroundColor: colors.card }]} onPress={() => setOpen(true)} activeOpacity={0.75} accessibilityLabel={t('common.searchFields', 'Arama alanları')}>
      ...
      <View style={styles.overlay}>
        ...
        <View style={[styles.panel, { backgroundColor: colors.bg, borderColor: colors.border }]}>
          <View style={styles.header}>
            ...
            <TouchableOpacity onPress={() => setOpen(false)}><Cancel01Icon size={21} color={colors.muted} /></TouchableOpacity>
          </View>
...
const styles = StyleSheet.create({ trigger: { width: 48, height: 48, borderRadius: 12, borderWidth: 1, alignItems: 'center', justifyContent: 'center' }, triggerCompact: { width: 42, height: 42 }, badge: { position: 'absolute', right: -3, top: -4, minWidth: 17, height: 17, paddingHorizontal: 3, borderRadius: 9, backgroundColor: '#db2777', alignItems: 'center', justifyContent: 'center' }, badgeText: { color: '#fff', fontSize: 9, fontWeight: '800' }, overlay: { flex: 1, backgroundColor: 'rgba(0,0,0,.58)', alignItems: 'center', justifyContent: 'center', padding: 18 }, panel: { width: '100%', maxWidth: 430, maxHeight: '75%', borderWidth: 1, borderRadius: 22, overflow: 'hidden' }, header: { flexDirection: 'row', gap: 10, alignItems: 'flex-start', padding: 16 }, headerIcon: { width: 38, height: 38, borderRadius: 13, alignItems: 'center', justifyContent: 'center' }, headerText: { flex: 1 }, title: { fontSize: 16, fontWeight: '800' }, help: { marginTop: 3, fontSize: 11, lineHeight: 16 }, search: { marginHorizontal: 14, marginBottom: 8, height: 42, borderWidth: 1, borderRadius: 12, paddingHorizontal: 12, flexDirection: 'row', alignItems: 'center', gap: 8 }, input: { flex: 1, fontSize: 13 }, list: { maxHeight: 350 }, grid: { padding: 10, flexDirection: 'row', flexWrap: 'wrap' }, option: { width: '50%', minHeight: 43, borderRadius: 12, paddingHorizontal: 10, flexDirection: 'row', alignItems: 'center', gap: 8 }, locked: { opacity: .55 }, check: { width: 20, height: 20, borderRadius: 10, borderWidth: 1, alignItems: 'center', justifyContent: 'center' }, optionText: { flex: 1, fontSize: 13, fontWeight: '600' }, footer: { minHeight: 48, borderTopWidth: 1, paddingHorizontal: 16, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, count: { fontSize: 11, fontWeight: '600' }, all: { fontSize: 12, fontWeight: '800' } });
