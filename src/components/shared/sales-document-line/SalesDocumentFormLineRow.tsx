...
import {
  formatSalesDocumentLineMoney,
  formatSalesDocumentLineQty,
  formatSalesDocumentLineRate,
} from "@/lib/salesDocumentLineDisplayFormat";
...
export interface SalesDocumentFormLineRowProps<T extends SalesDocumentLineFormState> {
  line: T;
  related?: boolean;
  isReadonly?: boolean;
...
function SalesDocumentFormLineRowComponent<T extends SalesDocumentLineFormState>({
  line,
  related = false,
  isReadonly = false,
  currencyLabel,
  ...
  onEdit,
  onDelete,
}: SalesDocumentFormLineRowProps<T>): React.ReactElement {
  ...
  const discountRates = [line.discountRate1, line.discountRate2, line.discountRate3]
    .filter((rate) => rate > 0)
    .map((rate) => `${formatSalesDocumentLineRate(rate)}%`);
        ...
        <View style={styles.topRow}>
          ...
          <View style={styles.topActions}>
            {!related ? (
              ...
              >
                <Text style={[styles.mainBadgeText, { color: accent }]}>
                  {t(`${translationPrefix}.main`)}
                </Text>
              </View>
            ) : null}
            {!isReadonly ? (
        ...
        <View style={[styles.bodyRow, hasImage && styles.bodyRowWithThumb]}>
          ...
          <View style={styles.bodyText}>
            <Text style={[styles.productName, { color: titleText }]} numberOfLines={2}>
              {line.productName || t(`${translationPrefix}.productNotSelected`)}
            </Text>
            {descriptionSummary ? (
        ...
        >
          <View style={styles.pricingTopRow}>
            <Text style={[styles.pricingQty, { color: titleText }]}>
              {formatSalesDocumentLineQty(line.quantity)}
            ...
            <Text style={[styles.pricingUnitPrice, { color: titleText }]}>
              {formatSalesDocumentLineMoney(line.unitPrice)}
              {currencyLabel ? (
        ...
        {needsApproval ? (
          <View style={[styles.approvalBanner, { backgroundColor: colors.warning + "18" }]}>
            ...
            <Text style={[styles.approvalText, { color: colors.warning }]}>
              {t(`${translationPrefix}.approvalRequired`)}
            </Text>
          </View>
        ) : null}
      </View>
...
export const SalesDocumentFormLineRow = memo(
  SalesDocumentFormLineRowComponent
) as typeof SalesDocumentFormLineRowComponent;
