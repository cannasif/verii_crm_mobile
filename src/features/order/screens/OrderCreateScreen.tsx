...
export function OrderCreateScreen(): React.ReactElement {
  ...
  const watchedActivityId = watch("order.activityId");
  ...
  const watchedSalesTypeDefinitionId = watch("order.salesTypeDefinitionId");
  const watchedOzelKod1 = watch("order.ozelKod1");
  const watchedGeneralDiscountRate = watch("order.generalDiscountRate");
  const watchedGeneralDiscountAmount = watch("order.generalDiscountAmount");
  ...
  useEffect(() => {
    ...
    if (getValues("order.deliveryDate") === nextDeliveryDate) return;
    setValue("order.deliveryDate", nextDeliveryDate, {
      shouldDirty: true,
      shouldValidate: true,
    });
  ...
  useEffect(() => {
    ...
    const currentOzelKod1 = getValues("order.ozelKod1");
  ...
  useEffect(() => {
    ...
    if (getValues("order.ozelKod2") === nextOzelKod2) return;
    setValue("order.ozelKod2", nextOzelKod2, {
      shouldDirty: false,
      shouldValidate: true,
    });
  ...
  useEffect(() => {
    if (!watchedCustomerId) {
      if (watchedActivityId != null) setValue("order.activityId", null);
      return;
    }
    if (
      ...
      setValue("order.activityId", null);
    }
  ...
  useEffect(() => {
    ...
    setValue("order.salesTypeDefinitionId", null);
  ...
  const handleOfferDateChange = useCallback(
    (_: DateTimePickerEvent, selectedDate?: Date) => {
      if (selectedDate) {
        ...
        setValue("order.offerDate", nextOfferDate);
        setValue("order.deliveryDate", addDaysToDateOnly(nextOfferDate, 21));
      }
    },
    [setValue]
  );
  ...
  const applyCurrencyChange = useCallback(
    ...
    [watchedCurrency, exchangeRates, erpRatesForOrder, currencyOptions, setValue, showToast]
  );
  ...
  const ensureDocumentExchangeRate = useCallback((): boolean => {
    ...
    return false;
  }, [watchedCurrency, exchangeRates, erpRatesForOrder, currencyOptions, showToast]);
  ...
  const handleProductSelectWithRelatedStocks = useCallback(
    async (stock: StockGetDto, relatedStockIds?: number[]) => {
      ...
      if (filteredRelations.length > 0 && relatedStocks.length > 0) {
        const relatedLines: OrderLineFormState[] = [];
  ...
  const onSubmit = useCallback(
    async (formData: CreateOrderSchema) => {
      ...
      const effectiveExchangeRatePayload = buildEffectiveExchangeRates(
        ...
        erpRatesForOrder,
        currencyOptions,
        formData.order.offerDate || new Date().toISOString().split("T")[0]
      );
      ...
      createOrder.mutate({
        order: {
          ...formData.order,
          offerType: normalizeOfferType(formData.order.offerType),
          generalDiscountRate: formData.order.generalDiscountRate ?? null,
          generalDiscountAmount: formData.order.generalDiscountAmount ?? null,
          erpProjectCode: formData.order.erpProjectCode ?? null,
          projectCode: formData.order.erpProjectCode ?? null,
          salesTypeDefinitionId: formData.order.salesTypeDefinitionId ?? null,
          deliveryMethod: formData.order.salesTypeDefinitionId != null
            ? String(formData.order.salesTypeDefinitionId)
            : null,
    ...
    [lines, exchangeRates, erpRatesForOrder, currencyOptions, notes, createOrder, setError, clearDraft]
  );
    ...
    <>
      ...
      <View style={[styles.container, { backgroundColor: mainBg }]}>
          ...
          <View style={[styles.section, { backgroundColor: shellBg, borderColor: sectionOutline }]}>
            ...
            <View style={styles.twoColumnRow}>
              <View style={styles.twoColumnItem}>
                <Controller
                  control={control}
                  name="order.salesTypeDefinitionId"
                  render={({ field: { value } }) => (
                    <View style={styles.fieldContainerTight}>
                      <Text style={[styles.labelCompact, { color: colors.textSecondary }]}>{t("order.deliveryMethod")}</Text>
                      <TouchableOpacity
              ...
              <View style={styles.twoColumnItem}>
                <Controller
                  control={control}
                  name="order.erpProjectCode"
                  render={({ field: { value } }) => (
                    <View style={styles.fieldContainerTight}>
                      <Text style={[styles.labelCompact, { color: colors.textSecondary }]}>{t("order.projectCode")}</Text>
                      <TouchableOpacity
                        style={[styles.pickerButton, styles.pickerShellCompact, { backgroundColor: innerBg, borderColor: innerBorder }]}
            ...
            <View style={styles.twoColumnRow}>
              <View style={styles.twoColumnItem}>
                <Controller
                  control={control}
                  name="order.generalDiscountRate"
                  render={({ field: { onChange, value } }) => (
                    <View style={styles.fieldContainerTight}>
                      <Text style={[styles.labelCompact, { color: colors.textSecondary }]}>{t("order.generalDiscountRate")}</Text>
                      <TextInput
              ...
              <View style={styles.twoColumnItem}>
                <Controller
                  control={control}
                  name="order.generalDiscountAmount"
                  render={({ field: { onChange, value } }) => (
                    <View style={styles.fieldContainerTight}>
                      <Text style={[styles.labelCompact, { color: colors.textSecondary }]}>{t("order.generalDiscountAmount")}</Text>
                      <TextInput
                        style={[styles.input, styles.inputCompact, { backgroundColor: innerBg, borderColor: innerBorder, color: colors.text }]}
                        value={value != null ? String(value) : ""}
                        onChangeText={(text) => {
            ...
            >
              ...
              <Text style={[styles.notesButtonText, { color: titleText }]}>
                {t("order.notesSection", "Sipariş Notları")}{notes.some((note) => note.trim()) ? ` (${notes.filter((note) => note.trim()).length})` : ""}
              </Text>
            </TouchableOpacity>
            <QuotationNotesModal
              visible={notesModalVisible}
              notes={notes}
              title={t("order.notesSection", "Sipariş Notları")}
          ...
          {lines.length > 0 && (
            <View style={[styles.section, { backgroundColor: shellBg, borderColor: sectionOutline }]}>
              ...
              {totals.generalDiscountAmount > 0 && (
                <View style={styles.summaryRow}>
                  <Text style={[styles.summaryLabel, { color: colors.textSecondary }]}>{t("order.generalDiscount")}:</Text>
        ...
        <PickerModal
          ...
          selectedValue={watch("order.erpProjectCode") ?? undefined}
          onSelect={(option) => { setValue("order.erpProjectCode", String(option.code ?? option.id)); setProjectCodeModalVisible(false); }}
          onClose={() => setProjectCodeModalVisible(false)}
          title={t("order.projectCode")}
          searchPlaceholder={t("order.projectCodeSearch")}
        />

        <PickerModal
          visible={salesTypeModalVisible}
          options={salesTypeList.map((item) => ({ id: item.id, name: item.name }))}
          selectedValue={watch("order.salesTypeDefinitionId") ?? undefined}
          onSelect={(option) => { setValue("order.salesTypeDefinitionId", option.id as number); setSalesTypeModalVisible(false); }}
        ...
        {watchedCustomerId ? (
          <PickerModal
            ...
            selectedValue={watch("order.activityId") ?? undefined}
            onSelect={(option) => {
              setValue("order.activityId", option.id as number, { shouldDirty: true, shouldValidate: true });
