...
interface OrderPreviewPdfDialogProps {
  visible: boolean;
  onClose: () => void;
  buildInput: (draft: boolean) => Promise<SalesDocumentPreviewPdfInput>;
  validateBeforeOpen?: () => string | null;
}

export function OrderPreviewPdfDialog({
  visible,
  onClose,
  buildInput,
  validateBeforeOpen,
}: OrderPreviewPdfDialogProps): React.ReactElement {
  ...
  const generatePdf = useCallback(
    async (draft: boolean) => {
      ...
      try {
        ...
        const fileUri = await createV3riiOrderPreviewPdf(input);
        ...
        const message = error instanceof Error ? error.message : t("order.exportPreview.error");
          ...
          <View style={[styles.header, { borderBottomColor: border }]}>
            <View style={styles.headerTextWrap}>
              <Text style={[styles.title, { color: colors.text }]}>
                {t("order.pdfExportTemplate.builtInTemplateTitle")}
              </Text>
              <Text style={[styles.subtitle, { color: muted }]}>
                {t("order.exportPreview.title")}
              </Text>
            </View>
            ...
            >
              {generatingKind === "draft" ? (
                ...
                <>
                  ...
                  <Text style={[styles.secondaryButtonText, { color: colors.accent }]}>
                    {t("order.exportPreview.draftPreview")}
                  </Text>
                </>
              )}
            </TouchableOpacity>
              ...
              >
                {generatingKind === "final" ? (
                  <>
                    ...
                    <Text style={styles.primaryButtonText}>{t("order.exportPreview.loading")}</Text>
                  </>
                ) : (
                  <>
          ...
          {isGenerating ? (
            ...
            <View style={[styles.infoCard, { borderColor: border }]}>
              <Text style={[styles.infoText, { color: muted }]}>
                {t("order.exportPreview.subtitle")}
              </Text>
            </View>
