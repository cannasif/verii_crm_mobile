...
interface OrderRowProps {
  ...
  onMenuPress: (e: GestureResponderEvent, order: OrderGetDto) => void;
}
...
function OrderRowComponent({
  ...
  const canShowCancellationReason =
    order.status === ApprovalStatus.CustomerCancelled && Boolean(cancellationReason);
        ...
        <View style={styles.customerContent}>
          ...
          {order.representativeName ? (
            <Text style={[styles.metaLine, { color: colors.muted }]} numberOfLines={1}>
              {t("order.representative")}: {order.representativeName}
            </Text>
          ) : null}
      ...
      <View style={styles.footerRow}>
        <View style={styles.totalBlock}>
          <Text style={[styles.totalLabel, { color: colors.muted }]}>
            {t("order.rowActions.totalAmount")}
          </Text>
        ...
        <Pressable
          ...
          accessibilityLabel={t("order.rowActions.menuTitle")}
        >
