...
export interface QuotationLinesSectionHeaderProps {
  lineCount: number;
  canAddLine: boolean;
  isReadonly?: boolean;
  onAddLine: () => void;
...
function QuotationLinesSectionHeaderComponent({
  lineCount,
  canAddLine,
  isReadonly = false,
  onAddLine,
}: QuotationLinesSectionHeaderProps): React.ReactElement {
  ...
  return (
    <View style={[styles.header, { borderBottomColor: borderColor }]}>
      <View style={styles.titleRow}>
        <Text style={[styles.title, { color: colors.text }]}>{t("quotation.lines")}</Text>
        {lineCount > 0 ? (
          <View
            style={[
              styles.countBadge,
...
export const QuotationLinesSectionHeader = memo(QuotationLinesSectionHeaderComponent);
