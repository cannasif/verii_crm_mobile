...
interface QuotationRowActionsSheetProps {
  visible: boolean;
  quotation: QuotationGetDto | null;
  isBusy: boolean;
  canRevise: boolean;
  isRevisionPending: boolean;
  onClose: () => void;
  onShareWhatsApp: (quotation: QuotationGetDto) => void;
  onShareGoogle: (quotation: QuotationGetDto) => void;
  onShareOutlook: (quotation: QuotationGetDto) => void;
  onDownload: (quotation: QuotationGetDto) => void;
  onRevise: (quotation: QuotationGetDto) => void;
}

export function QuotationRowActionsSheet({
  visible,
  quotation,
  isBusy,
  canRevise,
  ...
  onDownload,
  onRevise,
}: QuotationRowActionsSheetProps): React.ReactElement {
  ...
  const title =
    step === "share"
      ? t("quotation.rowActions.share")
      : t("quotation.rowActions.menuTitle");

  const subtitle =
    quotation?.offerNo?.trim() ||
    (quotation != null ? `#${quotation.id}` : "");
    ...
    <Modal visible={visible} transparent animationType="slide" onRequestClose={handleClose}>
      <View style={styles.overlay}>
        ...
        >
          ...
          {step === "main" && quotation != null ? (
            <View style={styles.options}>
              ...
              >
                ...
                <View style={styles.optionTextWrap}>
                  ...
                  <Text style={[styles.optionDesc, { color: palette.muted }]}>
                    {t("quotation.rowActions.shareDesc")}
                  </Text>
                </View>
              ...
              >
                ...
                <View style={styles.optionTextWrap}>
                  <Text style={[styles.optionTitle, { color: palette.text }]}>
                    {t("quotation.rowActions.download")}
                  </Text>
                  <Text style={[styles.optionDesc, { color: palette.muted }]}>
                    {t("quotation.rowActions.downloadDesc")}
                  </Text>
                </View>
              </TouchableOpacity>
              ...
              {canRevise ? (
                ...
                >
                  ...
                  <View style={styles.optionTextWrap}>
                    <Text style={[styles.optionTitle, { color: palette.text }]}>
                      {isRevisionPending
                        ? t("quotation.rowActions.revisePending")
                        : t("quotation.rowActions.revise")}
                    </Text>
                  </View>
                </TouchableOpacity>
              ) : null}
          ...
          {step === "share" && quotation != null ? (
            <View style={styles.options}>
              <TouchableOpacity
                style={[styles.optionBtn, { borderBottomColor: palette.sheetBorder }]}
                onPress={() => onShareWhatsApp(quotation)}
                disabled={isBusy}
                activeOpacity={0.72}
              >
                ...
                <View style={styles.optionTextWrap}>
                  <Text style={[styles.optionTitle, { color: palette.text }]}>
                    {t("quotation.rowActions.whatsapp")}
                  </Text>
                </View>
              </TouchableOpacity>
              ...
              <TouchableOpacity
                style={[styles.optionBtn, { borderBottomColor: palette.sheetBorder }]}
                onPress={() => onShareGoogle(quotation)}
                disabled={isBusy}
                activeOpacity={0.72}
              >
              ...
              <TouchableOpacity
                style={[styles.optionBtn, { borderBottomColor: "transparent" }]}
                onPress={() => onShareOutlook(quotation)}
                disabled={isBusy}
                activeOpacity={0.72}
              >
                ...
                <View style={styles.optionTextWrap}>
                  <Text style={[styles.optionTitle, { color: palette.text }]}>
                    {t("quotation.rowActions.outlook")}
                  </Text>
                </View>
              </TouchableOpacity>
            </View>
