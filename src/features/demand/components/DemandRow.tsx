...
interface DemandRowProps {
  ...
  onMenuPress: (e: GestureResponderEvent, demand: DemandGetDto) => void;
}
...
function DemandRowComponent({
  ...
  const currencyLabel = useMemo(() => resolveSalesDocumentRowCurrencyLabel(demand), [demand]);
  const amountText = useMemo(() => resolveSalesDocumentRowAmountText(demand), [demand]);
  const erpCleanupInfo = resolveErpCleanupInfo(demand, t);

  const showToast = useToastStore((state) => state.showToast);
  const cancellationReason = demand.cancellationReason?.trim();
  const canShowCancellationReason =
    demand.status === ApprovalStatus.CustomerCancelled && Boolean(cancellationReason);
    ...
    >
      <View style={styles.topRow}>
        <View style={styles.idColumn}>
          ...
          >
            <Text style={[styles.idText, { color: colors.brand }]}>#{demand.id}</Text>
          </View>
          {demand.offerNo ? (
        ...
        <View style={styles.customerContent}>
          ...
          {demand.representativeName ? (
            <Text style={[styles.metaLine, { color: colors.muted }]} numberOfLines={1}>
              {t("demand.representative")}: {demand.representativeName}
            </Text>
          ) : null}
      ...
      <View style={styles.footerRow}>
        <View style={styles.totalBlock}>
          <Text style={[styles.totalLabel, { color: colors.muted }]}>
            {t("demand.rowActions.totalAmount")}
          </Text>
        ...
        <Pressable
          ...
          accessibilityLabel={t("demand.rowActions.menuTitle")}
        >
