...
exports.buildBusinessCardPotentialMatchFilters = buildBusinessCardPotentialMatchFilters;
exports.scoreBusinessCardPotentialMatches = scoreBusinessCardPotentialMatches;
...
function websiteStem(value) {
    const domain = domainOf(value);
    return domain.split(".")[0] ?? "";
}
function phoneTail(value, size = 7) {
...
function buildBusinessCardPotentialMatchFilters(result) {
    const filters = [];
    const pushUnique = (filter) => {
        if (!filter.value)
            return;
...
function scoreBusinessCardPotentialMatches(result, customers) {
